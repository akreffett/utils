    ...
    def create_from_dict(cls, file_attributes):
        ...
        for key, value in file_attributes.items():
            if key == 'md5' and common_methods.test_value(value):
                ...
                packer = cybox_file_object.PackerType(Name=common_methods.create_element_from_dict(cybox_common.StringObjectAttributeType(datatype='String'), value))
                packer_list.add_Packer(packer)
                fileobj.set_Packer_List(packer_list)
            elif key == 'filename' and common_methods.test_value(value):
                ...
                filepath = common_methods.create_element_from_dict(cybox_file_object.FilePathType(datatype='String'),value.path)
                if hasattr(value, "fully_qualified"): filepath.set_fully_qualified(value.fully_qualified)
